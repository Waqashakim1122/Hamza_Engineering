...
interface CategoryTypesGridProps {
  ...
  products: (Product | any)[]; // Can be products or subcategory cards
    ...
    <section className="category-types-section">
      <div className="category-types__container">
        ...
        <header className="category-types__header">
          ...
          <div className="category-types__meta">
            <span className="category-types__count">
              {products.length} {isShowingSubcategories ? 'Types' : 'Models'} Available
            </span>
        ...
        <div className="category-types__grid">
          {products.map((item) => {
            // Determine if this is a subcategory or a product
            const isSubcat = item.isSubcategory || isShowingSubcategories;
            const linkHref = isSubcat 
            ...
            return (
              ...
              >
                <Link href={linkHref} className="type-card__link">
                  <div className="type-card__image-container">
                    ...
                    />
                    {/* Show product count badge for subcategories */}
                    {isSubcat && item.productCount && (
                      <div className="type-card__badge">
                        {item.productCount} {item.productCount === 1 ? 'Model' : 'Models'}
                      </div>
                    )}
                  </div>

                  <div className="type-card__info">
                    <h3 className="type-card__name">{item.name}</h3>
                    <p className="type-card__description">
                      {item.shortDescription}
                    </p>
                    <span className="type-card__action">
                      {isSubcat ? 'View Types →' : 'View Details →'}
                    </span>
                  </div>
                </Link>
              </motion.article>
